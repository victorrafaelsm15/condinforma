-- Correção de um bug introduzido em tenant_isolation_hardening_migration.sql:
-- as funções insert_execucao_publica/insert_ocorrencia_publica declaravam
-- "returns table (id text, inserted boolean)" — em PL/pgSQL, colunas de
-- RETURNS TABLE viram parâmetros OUT no escopo da função, e "id" colidia
-- com a coluna "id" de execucoes/ocorrencias usada dentro do próprio corpo
-- (erro real, medido: "column reference id is ambiguous", ambas as
-- funções falhavam 100% das vezes). Renomeia as colunas de saída pra algo
-- que não existe em nenhuma tabela, e ajusta os "returning ... into".
-- CREATE OR REPLACE não permite mudar o tipo de retorno — precisa dropar
-- antes.

drop function if exists insert_execucao_publica(text, text, text, text, integer, integer, jsonb, text, text, timestamptz);
...
drop function if exists insert_ocorrencia_publica(text, text, text, text, text, text, text, text, timestamptz);
